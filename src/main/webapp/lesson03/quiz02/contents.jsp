...
<section >
	<div class="main-info d-flex align-items-center">
		<img class="mx-3" src="<%=artistInfo.get("photo")%>" width="210px" height="210px" alt="아이유"/>
		<div class="mb-5 mt-0">
			<h1 class="font-weight-bold mb-3"><%=artistInfo.get("name")%></h1>
			<h3><%=artistInfo.get("agency") %></h3>
			<h3><%=artistInfo.get("debute") %></h3>
		</div>
	</div>
	<div>
		<h2 class="font-weight-bold mt-4 ml-2">곡 목록</h2>
		<table class="table text-center">
			...
			<tbody>
				...
				for(Map<String,Object> item: musicList){%>
					<tr>
						<td><%=item.get("id")%></td>
						<td><a href="quiz02_template.jsp?search=<%=item.get("title")%>"><%=item.get("title")%></a></td>
